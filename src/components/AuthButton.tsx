...
import { ShieldCheck, LogOut, User as UserIcon, Loader2 } from 'lucide-react';
...
export const AuthButton: React.FC = () => {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      console.warn('Google sign-in completed or dismissed:', e);
    } finally {
      setSigningIn(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="w-8 h-8 rounded-full border border-stone-200 dark:border-zinc-800 bg-stone-100 dark:bg-zinc-800 flex items-center justify-center">
        <Loader2 size={14} className="animate-spin text-stone-400 dark:text-zinc-500" />
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <button
        ...
        disabled={signingIn}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-stone-300/80 dark:border-zinc-700 bg-white dark:bg-zinc-900 hover:bg-stone-50 dark:hover:bg-zinc-800 text-xs font-mono font-medium text-stone-800 dark:text-zinc-200 transition-colors shadow-2xs cursor-pointer disabled:opacity-75"
      ...
      >
        ...
        <span>{signingIn ? (language === 'en' ? 'Signing in...' : 'Menghubungkan...') : (language === 'en' ? 'Sign In' : 'Masuk')}</span>
      </button>
    );
  }
